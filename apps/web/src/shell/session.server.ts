 ...
 * memberships in the auth service), the scope resolved from the URL, and the
 * grants the user holds there — computed once per request through the Alfiz
 * client over the auth service.
 */
...
import { call, type Services } from "./services.server.ts";
import { products } from "../registry.ts";

const clients = new WeakMap<Services["auth"], TpxClient>();

/** One Alfiz client per isolate; closures are cached with epoch revalidation against the auth service. */
export function getTpxClient(services: Services): TpxClient {
  const { auth } = services;
  let client = clients.get(auth);
  if (!client) {
    client = createTpxClient({
      getSubjectAccess: (principal) => auth.getSubjectAccess(principal),
      resolveAncestors: (scope) => auth.resolveAncestors(scope),
      epochHead: () => auth.epochHead(),
      epochSince: (seq, limit) => auth.epochSince(seq, limit),
    });
 ...
 */
async function grantsAtScope(
  services: Services,
  userId: string,
  scope: string,
  need: TpxKey | null,
): Promise<TpxKey[]> {
  const client = getTpxClient(services);
...
/**
 * Ingress: who (Clerk) → the auth service records them, claims invitations
 * and bootstraps a first tenant → which tenant (the remembered one, else the
 * first) → the grants they hold there. Membership is checked here against
 * what the auth service returned; a cookie can never name a tenant the user
 * is not in.
 */
export async function resolveTenantSession(
  ...
  let session = await call(services.auth.ensureUser({ userId: identity.userId }));
  if (!session.user || session.user.updatedAt < Date.now() - PROFILE_TTL_MS) {
    const profile = await fetchProfile(args, identity);
    session = await call(services.auth.ensureUser({ userId: identity.userId, profile }));
  }
  ...
  const tenant = await call(services.auth.findTenant(membership.id));
  if (!tenant) throw data({ error: "tenant not found" }, { status: 500 });
  const scope = tenantScope(tenant.id);
  const grants = await grantsAtScope(services, identity.userId, scope, null);
...
export async function resolveScopeSession(
  ...
  const resolved = await call(
    services.auth.resolveScope({ tenantId: session.tenant.id, projectSlug, environmentName }),
  );
  ...
  const grants = await grantsAtScope(services, session.identity.userId, scope, "tpx.workspace.projects.read");
...
/** What each service says about itself; products without a service are previews or hidden. */
export async function productCapabilities(
  services: Services,
  env: WebEnv,
): Promise<Record<string, ProductCapabilities | null>> {
  const preview = previewProducts(env);
  const out: Record<string, ProductCapabilities | null> = {};
  await Promise.all(
    products.map(async (manifest) => {
      const service = manifest.service ? services[manifest.service] : undefined;
      if (service) {
        try {
          out[manifest.id] = await service.capabilities();
          return;
        } catch (error) {
